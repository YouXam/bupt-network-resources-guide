...
+ #link("https://webvpn.bupt.edu.cn")[WebVPN]。这是网页版的 VPN，不需要下载客户端。需要使用企业微信扫码登录。
+ #link("https://libcon.bupt.edu.cn")[电子资源统一访问系统]。此系统虽然是用于在非校园网环境访问数据库，但也可以访问校园网环境下的普通资源。需要使用微信/企业微信扫码登录。
...
更详细的论坛使用指南、操作流程、介绍和使用规定等见#link("https://bbs.byr.cn/wiki/")[北邮人论坛使用指南]。
